...
trainset_gen = data_gen(list(zip(train_im[valid_size:],
                                 train_sparse[valid_size:],
                                 train_label[valid_size:])), batch_size)
# --------------------------------- helper funcs -----------------------------
# %%
...
for step in range(number_of_steps):
    ...
    _, totloss, pred, theta = sess.run([optimizer, total_loss,
                                        model.predict, model._sc_block.theta],
                                       {model.input: img,
...
Z_TSNE = []
labels_TSNE = []
TSNE_iter = iter(zip(test_im, test_label))
for i in range(400):
    img, label = next(TSNE_iter)
